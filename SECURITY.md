...
ランキングは匿名認証とRealtime Database Rulesで他ユーザーの上書きを制限しますが、問題判定とタイマーがクライアント内にあるため、記録の完全な正当性は保証できません。競技賞金など、高い改ざん耐性を必要とする用途には使用しないでください。

プレイヤー名とシードはランキングへ永続的に公開保存されます。個人情報、秘密情報、不適切な文字列を入力しないでください。
