...
ランキングはルーム削除後も`leaderboards`に残り、各匿名UIDについて30カテゴリーごとの自己ベストを保存します。利用者増加に応じてRealtime Databaseの保存量を監視してください。ランキングはクライアント計測の参考記録であり、Cloud Functionsを使わないSpark構成では完全な不正防止は行いません。
...
- RulesとHostingは新旧クライアントの互換性を確認し、`node scripts/emulator-spark-smoke.mjs`の成功後に同じリリースとして反映してください。
